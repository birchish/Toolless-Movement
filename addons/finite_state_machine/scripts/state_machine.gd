...
@export var debug_draw_offset : Vector2 = Vector2.ZERO: set = set_debug_draw_offset, get = get_debug_draw_offset
## Color of the text drawn if [member debug_draw] is assigned.
@export var debug_draw_color : Color = Color.WHITE: set = set_debug_draw_color, get = get_debug_draw_color
...
func _on_debug_draw_draw() -> void:
	...
	if OS.is_debug_build() and is_instance_valid(debug_draw) and is_instance_valid(current_state):
		var text_size : Vector2 = ThemeDB.fallback_font.get_string_size(current_state.name)
		var position : Vector2 = debug_draw_offset + Vector2(text_size.x * -0.5, 0.0)
